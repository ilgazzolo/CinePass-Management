package com.api.boleteria.repository;

import com.api.boleteria.model.Cinema;
import com.api.boleteria.model.TipoPantalla;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface ICinemaRepository extends JpaRepository<Cinema, Long> {
    List<Cinema> findByTipoPantalla(TipoPantalla tipoPantalla);
    List<Cinema> findByHabilitada(boolean habilitada);
    List<Cinema> findByCapacidadGreaterThan(Integer capacidad);
}
